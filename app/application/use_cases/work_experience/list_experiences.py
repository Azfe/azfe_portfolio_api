...
class ListExperiencesUseCase(
    IQueryUseCase[ListExperiencesRequest, WorkExperienceListResponse]
):
    """
    ...
    async def execute(
        self, request: ListExperiencesRequest
        ...
        experiences = await self.experience_repo.get_all_ordered(
            profile_id=request.profile_id, ascending=request.ascending
        )
