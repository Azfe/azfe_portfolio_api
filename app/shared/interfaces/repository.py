...
if TYPE_CHECKING:
    from app.domain.entities import (AdditionalTraining,  # noqa: F401
                                     Certification, ContactInformation,
                                     ContactMessage, Education, Language,
                                     Profile, ProgrammingLanguage, Project,
                                     Skill, SocialNetwork, Tool,
                                     WorkExperience)
...
class IOrderedRepository(IRepository[T], Generic[T]):
    ...
    @abstractmethod
    async def get_all_ordered(self, profile_id: str, ascending: bool = True) -> list[T]:
        """
...
class IUniqueNameRepository(IRepository[T], Generic[T]):
    ...
    @abstractmethod
    async def get_by_name(self, profile_id: str, name: str) -> T | None:
        """
